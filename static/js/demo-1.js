(function() {
    ...
    var pageWidth = jQuery('#box2d-demo').width();
    var pageHeight = jQuery('#box2d-demo').height();
    
    groundx = pageWidth/30/2;
    groundy = pageHeight/30;
	leftx = -1;
	lefty = pageHeight/30/2;
	rightx = pageWidth/30 + 1.1;
	righty = lefty;
	rightHeight = pageHeight/30;
    ...
    var init = {
        start: function(id) {
            ...
            setInterval(function() { add.random();},  200);
        ...
        surroundings: {
            rightWall: function() {
                add.box({
                    x: rightx, //25.7,        // 740 / 30 + 1.1
                    y:  righty, //6.3,        // 380px / 30 / 2
                    height: pageHeight/30, //12.6,   // 380px / 30
                    width:2,
                    isStatic: true
                });
            },
            ground: function() {
                add.box({
                    x: groundx, //12.3,        // 740 / 30 / 2
                    y:  groundy, //13.7,
                    height: 2,
                    width: pageWidth/30, //24.6,     // 740 / 30
                    isStatic: true
                });
            },
            leftWall: function() {
    ...
    window.setInterval(goPiece, 1000)

    function goPiece(){
    	//alert('adhfdsaf')
    	console.log('ground!!')
        var shapeOptions = {
                x: (canvas.width / SCALE) * (Math.random()*canvas.width),
                y: 0
            };
        add.random(shapeOptions);
